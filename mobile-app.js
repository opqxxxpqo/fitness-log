...
function WorkoutModal({ visible, title, draft, onChange, onPickImage, onCancel, onSubmit }) {
...
export default function App() {
  ...
  function beginEditWorkout(workout) {
    setEditingWorkoutId(workout.id);
    setDraft({
      name: workout.name,
      category: workout.category,
      defaultSets: String(workout.defaultSets),
      defaultReps: String(workout.defaultReps),
      defaultWeight: String(workout.defaultWeight),
      image: workout.image || ""
    });
  ...
  function submitWorkout() {
    ...
    const workout = { id: editingWorkoutId || createId(), name: draft.name.trim(), category: draft.category.trim(), defaultSets: draft.defaultSets || "4", defaultReps: draft.defaultReps || "12", defaultWeight: draft.defaultWeight || "20", image: draft.image };
    updateState((current) => ({
      ...current,
      workouts: editingWorkoutId ? current.workouts.map((item) => item.id === editingWorkoutId ? workout : item) : [...current.workouts, workout]
    }));
    setDraft(emptyDraft);
  ...
  function removeWorkout(workoutId) {
    Alert.alert("确认删除", "确定要删除这个训练项目吗？", [
      { text: "取消", style: "cancel" },
      {
        text: "删除",
        style: "destructive",
        onPress: () => updateState((current) => ({ ...current, workouts: current.workouts.filter((item) => item.id !== workoutId), activeSession: current.activeSession.activeWorkoutId === workoutId ? { ...current.activeSession, activeWorkoutId: null } : current.activeSession }))
      }
    ]);
  }
